...
describe('Favorites-icon componentis rendered', () => {
    ...
    test('Favorites-icon function is false', () => {
        const result = isFav(false);

        expect(result).toBeUndefined();
        expect(isFav).toHaveBeenCalledTimes(1);
        expect(isFav).toHaveBeenCalledWith(false);
    });
    ...
    test('Favorites-icon function CheckIsFav is true', () => {
    ...
    test('Favorites-icon function CheckIsFav is false', () => {
        const result = CheckIsFav(false);

        expect(result).toBeUndefined();
        expect(CheckIsFav).toHaveBeenCalledTimes(1);
        expect(CheckIsFav).toHaveBeenCalledWith(false);
    });
});
